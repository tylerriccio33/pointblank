...
print(validation.get_json_report())
```

We can use the `pre=` argument (available in every validation method) to mutate table for a specific
validation step.
...
validation = (
    pb.Validate(
        data=tbl,
        label="Mutating a table during interrogation"
    )
    .col_vals_between(
...
The validation plan can be quite large if you want it to be. Here, in two different validation
steps, we use the `pre=` argument to mutate the target table with a lambda.
...
validation = (
    ...
    .col_vals_ge(
        columns="a",
        value=20,
        pre=lambda dfn: dfn.with_columns(nw.col("a") * 20)
    )
    .col_vals_gt(
        columns="new",
        value=20,
        pre=lambda dfn: dfn.with_columns(new=nw.col("a") * 15)
    )
...
validation = (
    pb.Validate(
        data=pb.load_dataset(),
        tbl_name="small_table",
        label="Using column selector functions"
    )
...
validation = (
    pb.Validate(
        data=tbl,
        label="Column selectors: advanced"
    )
    ...
    .col_vals_regex(columns=pb.col("word"), pattern="a")
    .interrogate()
    )
...
validation = pb.Validate(small_table).col_count_match(count=8).interrogate()
...
# Use the `col_schema_match()` validation method to perform schema check
validation = (
...
Above, we obtained the rows that didn't pass validation in validation step 3 (a check on the
`session_duration` column). Using `frame=True` means we want the DataFrame directly instead of a
dictionary object.

Sundering data with `get_sundered_data()` really amounts to splitting the table based on the
validation results. Here, the target dataset is split into a table containing only the rows where
there were no row-level failures found during interrogation.
...
You could also use `type="fail"` to get the rows in the table where there was at least one test unit
failure.
