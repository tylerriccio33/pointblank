...
At the core of Pointblank is the validation plan. Its made up of rules, results, and steps.
...
Here's each validation component marked in the diagram summarized:

- validation steps: each step will execute a validation test
- validation rules: the configuration used for a given step
- validation results: the result of running a given step
...
Beyond just documenting the API, we've focused on providing our take on best practices. We have
dedicated sections on important strategies like troubleshooting common patterns and best
practices.

For example, a brief is a short piece of text that describes a validation step:

...provide an annotated image with briefs in a simple validation report table...

Notice the small brief text being pointed to in the first step of the validation report. Briefs
help readers interpret your validation.

I'm not just interested in your ability to specify briefs, but what makes them effective. In the
[Briefs]() article we provide a Best Practices section with five concrete guidelines
for using Briefs effectively.
