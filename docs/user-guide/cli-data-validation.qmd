...
Let’s scaffold a template script:

```bash
pb make-template my_validation.py
```

Edit `my_validation.py` to add your own data loading and validation logic. Here’s a simple example:

```python
import pointblank as pb

validation = (
    pb.Validate("worldcities.csv")
    .col_vals_not_null("city")
    .col_vals_gt("population", 0)
    .interrogate()
)
```

Now, run your validation script from the terminal:

```bash
pb run my_validation.py
```

![](/assets/pb-run-my-validation.png){width=100%}
...
### Overriding Data Sources and Output Options

You can override the data source in your script using the `--data` option—great for reusing scripts
with different datasets:

```bash
pb run my_validation.py --data different_cities.csv
```

Save the validation report as HTML or JSON for sharing or automation:

```bash
pb run my_validation.py --output-html report.html --output-json report.json
```

Use `--show-extract` or `--write-extract` to debug failing rows, just like with `pb validate`.

### Controlling Failure Behavior

Use `--fail-on` to control when the command should exit with an error, based on the severity of
validation failures:

```bash
pb run my_validation.py --fail-on error
```
