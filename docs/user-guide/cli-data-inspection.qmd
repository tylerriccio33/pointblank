...
That's 11 columns and while the all columns are shown (none in the middle are truncated from view)
we start to see some (necessary) instances of shortening via `…` in the column names and in the
displayed values. Let's now use the `--columns` with a set of column names:

![](/assets/pb-preview-game_revenue-column-names.png){width=100%}

Now, those few columns that are displayed no longer have to abbreviate their data values. This sort
of selective display becomes more necessary as the width of the terminal is reduced.
