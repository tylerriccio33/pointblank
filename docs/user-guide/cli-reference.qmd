...
>   distinct --check rows-complete pb validate data.csv --check col-vals-not-
>   null --column email --check col-vals-gt --column age --value 18
>
> Options:
...
>
>   # Multi-line with editor (supports multiple statements) pb pl --edit
>
>   # Multi-statement code example in editor: # csv = pl.read_csv('data.csv') #
>   result = csv.select(['name', 'age']).filter(pl.col('age') > 25)
>
>   # Multi-line with a specific editor pb pl --edit --editor nano pb pl --edit
>   --editor code pb pl --edit --editor micro
>
>   # From file pb pl --file query.py
>
>   # Piping to other pb commands pb pl
>   "pl.read_csv('data.csv').filter(pl.col('age') > 25)" --pipe | pb validate
>   --check rows-distinct pb pl --edit --pipe | pb preview --head 10 pb pl
>   --edit --pipe | pb scan --output-html report.html pb pl --edit --pipe | pb
>   missing --output-html missing_report.html
>
