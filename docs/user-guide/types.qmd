...
Pointblank provides a comprehensive suite of validation methods designed to help you verify
different aspects of your data. This article introduces the main categories of validation methods,
explains their purpose, and demonstrates how to use them with practical examples.

From checking individual column values against thresholds to validating table structure and
detecting duplicates, these validation methods form the foundation of any data quality workflow.
Each method is specifically designed to address common data quality concerns and can be combined to
create robust validation plans.
...
Should you need to check for the presence of individual columns (regardless of data type), the
...
As the report indicates, there are some incomplete rows in the table. We can narrow our check to
specific columns using the `columns_subset=` parameter:

```{python}
(
    pb.Validate(data=pb.load_dataset("small_table"))
    .rows_complete(columns_subset=["a", "b", "f"])
    .interrogate()
)
```

This validation checks if every row has non-Null values for the columns `a`, `b`, and `f`. Since
these particular columns don't have any missing values, all test units pass.

The [`rows_complete()`](https://posit-dev.github.io/pointblank/reference/Validate.rows_complete.html)
method is particularly useful for identifying data completeness issues that might affect downstream
analysis or processing, especially when certain columns must have values for the data to be useful.
...
- **Column value validations** to check individual values against thresholds
([`col_vals_gt()`](https://posit-dev.github.io/pointblank/reference/Validate.col_vals_gt.html),
[`col_vals_lt()`](https://posit-dev.github.io/pointblank/reference/Validate.col_vals_lt.html),
etc.)
- **Range validations** to verify values fall within acceptable bounds
([`col_vals_between()`](https://posit-dev.github.io/pointblank/reference/Validate.col_vals_between.html), [`col_vals_outside()`](https://posit-dev.github.io/pointblank/reference/Validate.col_vals_outside.html))
- **Set membership validations** to confirm values belong to predefined sets
([`col_vals_in_set()`](https://posit-dev.github.io/pointblank/reference/Validate.col_vals_in_set.html))
- **Missing value validations** to detect or require Null values
([`col_vals_null()`](https://posit-dev.github.io/pointblank/reference/Validate.col_vals_null.html),
[`col_vals_not_null()`](https://posit-dev.github.io/pointblank/reference/Validate.col_vals_not_null.html))
- **String pattern validations** using regular expressions
([`col_vals_regex()`](https://posit-dev.github.io/pointblank/reference/Validate.col_vals_regex.html))
- **Table structure validations** to verify schemas and column presence
([`col_schema_match()`](https://posit-dev.github.io/pointblank/reference/Validate.col_schema_match.html),
[`col_exists()`](https://posit-dev.github.io/pointblank/reference/Validate.col_exists.html))
- **Row and column count validations** to confirm expected dimensions
...
- **Row-based validations** to check for duplicates or completeness
([`rows_distinct()`](https://posit-dev.github.io/pointblank/reference/Validate.rows_distinct.html),
[`rows_complete()`](https://posit-dev.github.io/pointblank/reference/Validate.rows_complete.html))

Each validation method can be combined with others to build a comprehensive data quality plan
tailored to your specific requirements. As your validation needs grow more complex, you can enhance
these methods with:

- [Column selection patterns](column-selection-patterns.qmd) for targeting multiple columns efficiently
- [Thresholds](thresholds.qmd) to set acceptable failure levels
- [Actions](actions.qmd) to respond to validation failures
- [Preprocessing](preprocessing.qmd) to transform data before validation
- [Segmentation](segmentation.qmd) to apply validations to specific data subsets

For a complete reference of all validation methods with detailed parameter descriptions and
examples, see the [API Reference](https://posit-dev.github.io/pointblank/reference/). The next
article in this guide covers [column selection patterns](column-selection-patterns.qmd) to help you
target validation methods at multiple columns efficiently.
