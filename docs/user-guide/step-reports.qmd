...
```{python}
first_step_report = validation.get_step_report(i=1)
...
```{python}
# Get a step report with a subset of columns
columns_report = validation.get_step_report(
    i=2,
...
limited_report = validation.get_step_report(
    i=2,
...
```{python}
# Customize the report header with some Markdown formatting
custom_header_report = validation.get_step_report(
    i=2, header="Category Values Validation: *Critical Analysis*"
)
...
```{python}
# Keep the details but customize the title
custom_title_report = validation.get_step_report(
    i=2,
...
```{python}
# Add additional context while keeping both title and details
context_report = validation.get_step_report(
    i=2,
    header=(
        "{title}<br>"
...
You could use a HTML and CSS to *really* do a lot of customization:

```{python}
# Rearrange elements with custom formatting
rearranged_report = validation.get_step_report(
    i=2,
    header=(
        "VALIDATION SUMMARY\n\n{details}\n\n"
...
no_header_report = validation.get_step_report(
    i=2,
    header=None
)
no_header_report
...
focused_report = validation.get_step_report(
    i=2,
