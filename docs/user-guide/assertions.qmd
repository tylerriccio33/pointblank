...
As you can see from the report, some values failed the validation. The brief message is displayed in
the validation report, making it clear what condition wasn't met.
...
validation = (
    pb.Validate(problem_data, thresholds=(0.1, 0.2, 0.3))
    .col_vals_gt(
        columns="id", value=0,
