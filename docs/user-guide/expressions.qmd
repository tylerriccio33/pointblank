...
1. `col_vals_expr()`: For validating data using custom expressions
2. `conjointly()`: For applying multiple validation steps simultaneously
3. `specially()`: For implementing fully custom validation functions
...
Expressions in `col_vals_expr()` are specified as strings and follow Python syntax with a few
...
The key difference between using `conjointly()` and adding multiple individual validation steps is
how test units are counted:
...
- use built-in methods (`col_vals_gt()`, etc.) for simple validations
- use `col_vals_expr()` for moderately complex logic
- use `conjointly()` for multiple related conditions
- use `specially()` only for highly custom validation logic
...
- `col_vals_expr()` allows you to validate data using custom expressions
- `conjointly()` lets you apply multiple validation steps simultaneously
- `specially()` enables fully custom validation functions
