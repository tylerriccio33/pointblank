...
in the body of a function to be used within
[`Actions`](https://posit-dev.github.io/pointblank/reference/Actions.html). This provides useful
...
## Setting Default Actions and Controlling Action Behavior

The [`Actions`](https://posit-dev.github.io/pointblank/reference/Actions.html) class has two
additional parameters that provide more control over how actions are executed:
...
For example, some validation steps like
[`col_vals_gt()`](https://posit-dev.github.io/pointblank/reference/Validate.col_vals_gt.html)
provide a `value` field that can be accessed with `{value}` in string templates, while others like
[`col_exists()`](https://posit-dev.github.io/pointblank/reference/Validate.col_exists.html) don't
...
`final_actions=` but here it's integrated into the user's `comprehensive_report()` function to
provide finer control with conditional logic.
...
The flexible design of [`Actions`](https://posit-dev.github.io/pointblank/reference/Actions.html)
and [`FinalActions`](https://posit-dev.github.io/pointblank/reference/FinalActions.html) allows you
to start simple with basic console messages and gradually build up to complex validation workflows
with conditional logic, custom reporting, and integrations with other systems like Slack, email, or
logging services.
