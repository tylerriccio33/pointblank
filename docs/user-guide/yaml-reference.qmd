...
```yaml
- specially:
    fn:                                # REQUIRED: Custom validation function
      python: |
        lambda df: custom_validation_logic(df)
    thresholds:                        # OPTIONAL: Step-level thresholds
      warning: 0.1
    actions:                           # OPTIONAL: Step-level actions
      warning: "Custom message"
...
actions:
  error:
    python: |
      lambda: send_alert("Validation failed")
```

You can also use the shortcut syntax for simple lambda expressions:
...
Common YAML validation errors and their solutions:

- "YAML must contain 'tbl' field": add required `tbl` key
- "YAML must contain 'steps' field": add required `steps` list
- "Unknown validation method": check method name spelling
- "Threshold must be non-negative": use positive threshold values
- "Failed to load data source": verify data file path/dataset name
- "Invalid YAML syntax": check YAML formatting and indentation

Use `validate_yaml()` to catch configuration errors before execution.
