...
stakeholders. For example, a failure can be seen in the second row (there's a CSV button). A data
quality stakeholder could click this to download a CSV of the failing rows.
...
- **execution**: the [`interrogate()`](https://posit-dev.github.io/pointblank/reference/Validate.interrogate.html)
...
validation step can test that each value in a column is less than a specified number. And the key
finding that's reported in each step is the number of *test units* that pass or fail.
...
- [Column Selection Patterns](columns.qmd): Techniques for targeting specific columns
