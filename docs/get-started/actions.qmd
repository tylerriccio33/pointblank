...
Actions are combined with thresholds and they allow you easily write text to the console or
execute custom functions. As an example, when testing a column for values that should always be
greater than `2` with `col_vals_gt()` you might want some text emitted to the console when any
failing test units are found.

Let's look at an example on how this works in practice. The following validation plan contains a
single step (using `col_vals_gt()`) where the `thresholds=` and `actions=` parameters are set.
...
level (i.e., set in `Validate`) or at the step level. The only thing to be aware of is that local
settings of either take precedence.
