...
The collection of validation methods in Pointblank allow you to express all sorts of checks of your
...
values in an different column. Here are all the validation methods of this type and comparison
checks they make:
...
These types of checks are typically numeric checks, so the column supplied in `columns=` is numeric
along with the value or column referenced in `value=`.
...
Schema validation is performed with
[`col_schema_match()`](https://posit-dev.github.io/pointblank/reference/Validate.col_schema_match.html)
and it checks whether the structure of a table matches an expected schema. It requires an
expectation of the table schema and that can be generated with the
[`Schema`](https://posit-dev.github.io/pointblank/reference/Schema.html) class. Here is a simple
example that checks whether a schema checking only column names is a match on the target table.
...
Any `col_schema_match()` validation step will only have a single test unit. We can see in the above
...
Both of these regular expression checks resulted in matches down columns `b` and `f`.
